...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.0, top_p=1.0, tokens=800, freq_pen=0.0, pres_pen=0.0, stop=['USER:', 'BENJI:']):
    max_retry = 5
    retry = 0
    ...
    while True:
        try:
            ...
            if not os.path.exists('interview_output_logs'):
                os.makedirs('interview_output_logs')
            save_file('interview_output_logs/%s' % filename, prompt + '\n\n==========\n\n' + text)
            return text
...
if __name__ == '__main__':
    ...
    def chat(input_text: InputText):
        ...
        memories = fetch_memories(vector, conversation, 30)  # pull episodic memories
        ...
        prompt = open_file('prompt_interviewer.txt').replace('<<NOTES>>', notes).replace('<<CONVERSATION>>', recent)
